...
\begin{enumerate}
    ...
    \begin{equation*}
        \Phi_1\equiv q_{0a} \vee q_{0na}
    \end{equation*}
    %2
    \item Un état est exclusivement acceptant ou non-acceptant :
    \begin{equation*}
        \Phi_2\equiv\bigwedge_{\substack{q\in Q \\ i\in \{1,\dots,k\}}} q_{ia} \rightarrow \neg q_{ina}\equiv
        \bigwedge_{\substack{q\in Q \\ i\in \{1,\dots,k\}}} \neg q_{ia} \vee \neg q_{ina}
    \end{equation*}
    %3
    \item Chaque état a au plus une transition par lettre de l'alphabet :
    \begin{equation*}
        \Phi_3\equiv\bigwedge_{\substack{l\in \Sigma\\i,j,q \in Q}} d_{i,l,j} \rightarrow \neg d_{i,l,q}\equiv
    ...
    \begin{equation*}
        \Phi_4\equiv\bigwedge_{\substack{t=len(m)\\q \in Q\\ m \in P\\ i \in \{1,\dots,k\}}} e_{m,i,t} \rightarrow q_{ia} \equiv
    ...
    \begin{equation*}
        \Phi_5\equiv\bigwedge_{\substack{i,j \in Q\\l \in \Sigma\\m \in P\\t \in \{0,\dots,len(m)\}\\l=m[t]}} (e_{m,i,t} \wedge d_{i,l,j} \rightarrow e_{m,j,t+1}) \equiv
    ...
    \begin{equation*}
        \Phi_6\equiv\bigwedge_{\substack{i,j \in Q\\l \in \Sigma\\m \in P\\t \in \{0,\dots,len(m)\}\\l=m[t]}} (e_{m,i,t} \wedge e_{m,j,t+1} \rightarrow d_{i,l,j})\equiv
        \bigwedge_{\substack{i,j \in Q\\l \in \Sigma\\m \in P\\t \in \{0,\dots,len(m)\}\\l=m[t]}} (\neg e_{m,i,t} \vee \neg e_{m,j,t+1} \vee d_{i,l,j})
    \end{equation*}
    %7
    \item Toutes les exécutions sur les mots de N doivent se terminer sur un état non-acceptant :
    \begin{equation*}
        \Phi_7\equiv\bigwedge_{\substack{m \in N\\i \in \{1,\dots,k\}\\t=len(m)}\\q\in Q} e_{m,i,t} \rightarrow q_{ina}\equiv
    ...
    \begin{equation*}
        \Phi_8\equiv\bigwedge{\substack{m\in P}}\bigvee_{\substack{t\in\{0,\dots,len(word)\}\\i\in \{1,\dots,k\}}} e_{m,i,t+1}
    \end{equation*}
    %9
    \item Toutes les exécutions doivent commencer à l'état initial :
    \begin{equation*}
        \Phi_9\equiv\bigwedge_{\substack{m\in P\cup N}} e_{m,1,0}
    \end{equation*}
\end{enumerate}
La formule en logique propositionelle finale est :
\begin{equation*}
    \Phi\equiv\bigwedge_{i=1,\dots,9} \Phi_i
\end{equation*}
...
\end{enumerate}
Etant donné que l'automate est fini, il existe un nombre fini d'états, la borne supérieure est toujours trouvée et il est possible d'itérer 
sur $k\in\{0,\dots,\infty\}$.\\
Finalement, il suffit d'utiliser les contraintes précédemment définies (cf. \ref{sub:contraintes}) afin de trouver
