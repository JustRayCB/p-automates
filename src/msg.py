...
def _get_d_transitions_from_model(
    model: list[int], alphabet: str, k: int
...
def _get_nd_transitions_from_model(
    model: list[int], alphabet: str, k: int
) -> dict[tuple[int, {str}], int]:
    """Retourne l'ensemble des transitions présentes dans le modèle"""
    transitions = {f"q{i}": dict() for i in range(k) if p_id(i) in model}
...
def _gen_cnf(
    constraints: list, alphabet: str, pos: list[str], neg: list[str], k: int, **args
) -> CNF:
...
def reverse(v: int) -> tuple:
    ...
    if tup[0] == P_ID:
        ...
        ret += f"t[{tup[1]},{tup[2]},{tup[3]}]"
    elif tup[0] == V_ID:
        ret += f"v[{tup[1]},{tup[2]},{tup[3]}]"
    elif tup[0] == X_ID:
        ret += f"x[{tup[1]},{tup[2]},{tup[3]}]"
    elif tup[0] == Y_ID:
        ret += f"y[{tup[1]},{tup[2]},{tup[3]},{tup[4]}]"
    return ret
